	...
	if message_text:lower():find("^!setorgname") ~= nil then
		...
		system.fiber(function(callback)
			...
			if string.len(input) > 32 then
				...
				add_to_history("Admin", "You must specify name Example, !setorgname ~r~ Red Ceo Name", "INFO")
			end
		end)
	return
	end
	if message_text:lower():find("^!setmcname") ~= nil then
		...
		system.fiber(function(callback)
			...
			if string.len(input) > 32 then
				...
				add_to_history("Admin", "You must specify name Example, !setmcname ~g~ This is a long name with a wanted star ~ws~", "INFO")
			end
		end)
	return
	end
